...
export default function CreateVideoconferencePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Falha ao criar videoconferência');
      }

      setSuccess(true);
      
      // Redirect to videoconference list after successful creation
      setTimeout(() => {
        router.push('/teacher/videoconference');
      }, 2000);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro desconhecido');
      console.error('Erro ao criar videoconferência:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <TeacherLayout>
      <div className="container mx-auto py-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Nova Videoconferência</h1>
          <Button 
            variant="outline"
          ...
          <CardContent>
            ...
            {success && (
              <div className="bg-green-50 text-green-700 p-4 rounded-md mb-6">
                <p>Videoconferência criada com sucesso! Redirecionando...</p>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                {loadingPlatforms && <p className="text-sm text-gray-500">Carregando plataformas...</p>}
              </div>
